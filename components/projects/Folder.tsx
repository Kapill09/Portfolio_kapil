...
export function Folder() {
  ...
  return (
    ...
    >
      ...
      >
        <div className="absolute inset-x-[3%] bottom-[1.5%] top-[13%] rounded-[clamp(28px,4vw,48px)] border border-[#d9d9d6] bg-[#f8f8f6] shadow-[0_24px_70px_rgba(32,73,95,.16),inset_0_0_0_8px_rgba(255,255,255,.72)] [html[data-sky-state=sunset]_&]:border-slate-700/80 [html[data-sky-state=sunset]_&]:bg-slate-900/95 [html[data-sky-state=sunset]_&]:shadow-[0_24px_70px_rgba(2,6,23,.42),inset_0_0_0_8px_rgba(30,41,59,.42)] [html[data-sky-state=night]_&]:border-slate-700/80 [html[data-sky-state=night]_&]:bg-slate-900/95 [html[data-sky-state=night]_&]:shadow-[0_24px_70px_rgba(2,6,23,.42),inset_0_0_0_8px_rgba(30,41,59,.42)]" />
        <div className="absolute inset-x-[4.15%] bottom-[3%] top-[15%] rounded-[clamp(23px,3.5vw,41px)] border-2 border-dashed border-[#c8c8c3] [html[data-sky-state=sunset]_&]:border-slate-600/70 [html[data-sky-state=night]_&]:border-slate-600/70" />
        ...
        <motion.div
          className="absolute inset-x-[3%] bottom-[1.5%] z-40 h-[53%] overflow-hidden rounded-b-[clamp(28px,4vw,48px)] rounded-t-[clamp(18px,2.5vw,30px)] border border-[#d3d3cf] bg-[#fafaf8] shadow-[0_-9px_20px_rgba(45,67,78,.08),inset_0_1px_0_white] [html[data-sky-state=sunset]_&]:border-slate-700/80 [html[data-sky-state=sunset]_&]:bg-slate-900 [html[data-sky-state=sunset]_&]:shadow-[0_-9px_20px_rgba(2,6,23,.34),inset_0_1px_0_rgba(148,163,184,.18)] [html[data-sky-state=night]_&]:border-slate-700/80 [html[data-sky-state=night]_&]:bg-slate-900 [html[data-sky-state=night]_&]:shadow-[0_-9px_20px_rgba(2,6,23,.34),inset_0_1px_0_rgba(148,163,184,.18)]"
          animate={{ y: isActive && !prefersReducedMotion ? 4 : 0 }}
          transition={{ type: 'spring', stiffness: 150, damping: 22, mass: 0.8 }}
        >
          <div className="absolute inset-0 opacity-80 [background-image:linear-gradient(#dcdedc_1px,transparent_1px),linear-gradient(90deg,#dcdedc_1px,transparent_1px)] [background-size:clamp(12px,1.6vw,20px)_clamp(12px,1.6vw,20px)] [html[data-sky-state=sunset]_&]:opacity-70 [html[data-sky-state=sunset]_&]:[background-image:linear-gradient(rgba(71,85,105,.42)_1px,transparent_1px),linear-gradient(90deg,rgba(71,85,105,.42)_1px,transparent_1px)] [html[data-sky-state=night]_&]:opacity-70 [html[data-sky-state=night]_&]:[background-image:linear-gradient(rgba(71,85,105,.42)_1px,transparent_1px),linear-gradient(90deg,rgba(71,85,105,.42)_1px,transparent_1px)]" />
          <div className="absolute inset-[10px] rounded-b-[clamp(20px,3.2vw,38px)] rounded-t-[clamp(12px,2vw,22px)] border-2 border-dashed border-[#c6c6c1] [html[data-sky-state=sunset]_&]:border-slate-600/70 [html[data-sky-state=night]_&]:border-slate-600/70" />

          <div className="absolute left-[4%] top-[6%] z-10 inline-flex items-center gap-2 rounded-full border-2 border-dashed border-[#bdbdb8] bg-[#fafaf8] px-[clamp(13px,2vw,24px)] py-[clamp(7px,1vw,12px)] text-[clamp(12px,1.6vw,20px)] font-bold text-[#24323b] shadow-[0_3px_0_#cfcfca] transition-transform group-hover:-translate-y-0.5 [html[data-sky-state=sunset]_&]:border-slate-500/70 [html[data-sky-state=sunset]_&]:bg-slate-800/95 [html[data-sky-state=sunset]_&]:text-slate-100 [html[data-sky-state=sunset]_&]:shadow-[0_3px_0_rgba(15,23,42,.9)] [html[data-sky-state=night]_&]:border-slate-500/70 [html[data-sky-state=night]_&]:bg-slate-800/95 [html[data-sky-state=night]_&]:text-slate-100 [html[data-sky-state=night]_&]:shadow-[0_3px_0_rgba(15,23,42,.9)]">
            View my works
